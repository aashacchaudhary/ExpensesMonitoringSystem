...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{unnamed.jpg}
\end{figure}
    
    {\large Submitted by:}\\[0.5cm]
    {\large \textbf{Sangam Panday (Roll No.)}}\\
    {\large \textbf{Sandesh Neupane (Roll No.)}}\\
    {\large \textbf{Spriha Bhattarai (Roll No.)}}\\[1.5cm]
...
This project demonstrates the practical application of artificial intelligence and computer vision in solving real-world traffic monitoring challenges specific to Nepali urban and highway environments. By developing a system trained on Nepali traffic datasets that can automatically recognize and categorize local vehicle types—including auto-rickshaws, motorcycles, and various bus configurations—the project highlights how custom deep learning models can be adapted for regional contexts. This localization approach enables Nepal to implement cost-effective, open-source traffic monitoring solutions tailored to local transportation needs rather than relying solely on generic international systems.
...
\begin{itemize}
    ...
    \item \textbf{Insufficient documentation on regional adaptation:} While theoretical research on transfer learning exists, practical implementations for adapting systems to South Asian contexts like Nepal are less documented.
...
\subsection{Flask}
Flask is a lightweight Python web framework used to develop the backend of this system. It manages routing, handles HTTP requests, processes uploaded files, and provides rapid development capabilities. Flask's flexibility allows for easy integration with machine learning models.
...
YOLOv8 is a state-of-the-art deep learning model for real-time object detection. It is used in this project to detect vehicles in images and video frames. The pre-trained YOLOv8n (nano) model provides a good balance between accuracy and processing speed.
...
\subsection{OpenCV}
OpenCV (Open Source Computer Vision Library) is used for video processing, frame extraction, image manipulation, and visualization of detection results with bounding boxes and labels.

\subsection{DeepSORT (Deep Learning SORT)}
DeepSORT tracker is used for multi-object tracking, enabling consistent vehicle ID tracking across video frames. This is particularly important for traffic monitoring to track individual vehicles through their journey across the traffic area.

\subsection{Pandas}
Pandas is used for data manipulation, analysis, and exporting results to CSV format, enabling comprehensive reporting and downstream analysis of Nepali traffic data.

\subsection{PyTorch}
PyTorch is the underlying deep learning framework that powers YOLOv8 and the custom Nepali vehicle classifier, providing efficient tensor computations and GPU acceleration capabilities.
...
\begin{itemize}
    ...
    \item It should maintain consistent vehicle tracking across frames using DeepSORT.
...
% ------------ CHAPTER 5 -----------------
\usepackage{float} % Add this in your preamble for [H] to work
...
\begin{itemize}
    ...
    \item \textbf{Machine Learning Models:} YOLOv8 for detection and custom Nepali vehicle classifier (vehicle\_classifier\_final.pt) for classification.
    \item \textbf{Tracking Engine:} DeepSORT for maintaining consistent vehicle tracking across frames.
    \item \textbf{Data Processing:} OpenCV for frame extraction and image processing.
...
Extracts frames from video files using OpenCV. Processes each frame sequentially for vehicle detection and classification.

\subsection{Detection Module}
Uses YOLOv8 model to detect vehicles in each frame, providing bounding box coordinates and confidence scores.
...
Implements DeepSORT tracker to maintain consistent vehicle IDs across frames and track vehicle trajectories.
...
Exports results to CSV format with columns for frame number, vehicle type, confidence, bounding box coordinates, vehicle color, and trajectory information.
...
\begin{itemize}
    ...
    \item \textbf{Models Directory:} Contains trained machine learning models including the custom Nepali vehicle classifier (vehicle\_classifier\_final.pt) and YOLOv8n model.
...
The system uses a custom-trained Nepali vehicle classifier model (vehicle\_classifier\_final.pt) that has been specifically trained on Nepali traffic datasets, along with YOLOv8 for detection. For each uploaded video or image:
\begin{itemize}
    \item Video files are split into frames using OpenCV.
    \item Each frame is passed to YOLOv8 for vehicle detection.
    \item DeepSORT tracker maintains consistent vehicle IDs across frames.
...
\begin{itemize}
    ...
    \item Setup Python environment and install dependencies (Flask, OpenCV, PyTorch, Pandas, DeepSORT).
    ...
    \item Integrate YOLOv8 model for vehicle detection.
    \item Develop and train custom Nepali vehicle classifier model.
    \item Develop video processing pipeline using OpenCV.
    \item Implement DeepSORT tracker for vehicle tracking.
...
The YOLOv8 model achieved high accuracy in detecting vehicles across different Nepali traffic scenarios with varying lighting conditions, distances, and angles typical of Nepali cities.
...
DeepSORT tracker successfully maintained vehicle identities across frames, enabling accurate trajectory tracking and vehicle-level analytics for Nepali traffic.
...
\begin{itemize}
    ...
    \item Generating comprehensive CSV reports with detailed Nepali vehicle analysis.
    \item Demonstrating the practical application of AI and computer vision in solving real-world Nepali traffic monitoring challenges.
\end{itemize}
